...
- `PATCH /api/auth/me` updates name/email/password for whoever is currently authenticated - works for both roles since it sits under the same `/api/auth/**` matcher as `DELETE /api/auth/me` (permitAll, the handler itself checks `Authentication`). If you touch this: changing the email must re-persist the `SecurityContext` with a fresh `UserDetails` (see `AuthService.updateProfile`) - the session's principal is the email, so leaving it stale breaks every subsequent request on that session.
- **Three roles now**: `CUSTOMER`, `ADMIN`, `STAFF` (added Sprint 7). STAFF has the same `/api/admin/**` access as ADMIN except staff-account management itself (`/api/admin/staff/**`, `AdminStaffController`), which stays ADMIN-only - see SecurityConfig's ordering (the narrower `staff/**` rule must come before the broader `anyRequest().hasAnyRole(...)` one). STAFF accounts are only ever created by an ADMIN via `POST /api/admin/staff` (name+email, random unusable password, immediately sends the same password-reset email as the public forgot-password flow) - there's no self-service staff signup.
- **Session invalidation on staff removal**: deleting a STAFF row (`AuthService.deleteStaff`) also kills their live Redis session(s) via `FindByIndexNameSessionRepository.findByPrincipalName(...)` - otherwise an already-logged-in staff member keeps admin access until their session naturally expires. Requires `spring.session.data.redis.repository-type=indexed` in `application.properties` - note the exact property name: the Spring Boot 4.1.1 configuration-metadata docs also list a `spring.session.redis.repository-type` alias, but the actual `@ConditionalOnProperty` on `SessionDataRedisAutoConfiguration`'s indexed-config class only reads the `session.data.redis` spelling (verified against the class bytecode after the shorter name silently did nothing).
...
- **Mobile layout gotcha (`position: absolute` inside a wrapping flex row)**: `Header.tsx`'s top row wraps onto multiple lines on narrow viewports (logo+nav, search, icons each end up on their own line). An `absolute`-positioned dropdown anchored to a small inner wrapper (`relative` on just the button, not the row) lands wherever that wrapper ends up after wrapping, not at the visual edge of the screen - see the mini-cart flyout bug in `docs/backlog.md`'s "Mobile-Responsive-Audit" entry. Anchor `relative` on the outermost full-width container instead when a dropdown needs to stay pinned to the actual screen edge.
- **Mobile layout gotcha (fixed bottom bars)**: a `fixed inset-x-0 bottom-0` bar (see `ProductPage.tsx`'s mobile add-to-cart bar) always occupies the same band of the viewport regardless of scroll position - on a short/narrow phone, real content (e.g. the product title, if the image alone nearly fills the screen) can end up rendering right underneath it on initial load. Don't just add bottom padding to the page (that only protects the very end of the page) - gate the bar's visibility on an `IntersectionObserver` watching whatever content it would otherwise cover.
...
- Reviews: `GET /api/products/{id}/reviews` is public. `POST /api/products/{id}/reviews` (added post-Sprint-6) requires being logged in, having actually bought the product (`ReviewRepository.hasPurchased`, joins `order_items`/`orders`), and not having reviewed it already (`hasReviewed`) - 403/409 respectively, otherwise 201. The form lives inline on `ProductPage`.
...
- Icons: no icon library - `frontend/src/components/icons.tsx` has two small hand-written SVG components (`HeartIcon`, `BagIcon`) instead of Unicode emoji, since emoji glyphs vary by OS/browser font. Add new icons the same way rather than pulling in an icon package for one or two glyphs.
...
- **CI** (`.github/workflows/ci.yml`): `lint-frontend`/`lint-backend` → `test-frontend`/`test-backend` → `e2e` → `security-scan` → `deploy` (active, see below). `test-backend` needs no special Docker setup on the runner (see above) - `ubuntu-latest` already has a Docker daemon `./mvnw test` can reach directly.
- **e2e project split**: `npx playwright test` runs as two separate steps - chromium/webkit/mobile-chrome/mobile-safari as a hard gate, then a firefox-only run with `continue-on-error: true`. Firefox has an unresolved, intermittent flake (register/login/cart flows occasionally time out under this runner's load, not reproduced on any other engine) - see the "Nachträge nach Sprint 6" entry in `docs/backlog.md` before assuming a Firefox-only CI failure means real app breakage.
- **Security scan**: `npm audit --audit-level=high` (frontend), OWASP `dependency-check-maven` invoked ad hoc (`./mvnw org.owasp:dependency-check-maven:13.0.0:check`, not wired into the normal build lifecycle - it's slow and only useful in CI), and `aquasecurity/trivy-action` against the built `almo-backend`/`almo-frontend` images. The OWASP scan needs an `NVD_API_KEY` repo secret (free, register at https://nvd.nist.gov/developers/request-an-api-key, **must be confirmed via the email link before NVD accepts it** - an unconfirmed/invalid key fails with `"Invalid apiKey."`) to avoid NVD's unauthenticated rate limit - the step's `continue-on-error` is unconditional (not just "key is empty") so an NVD-side problem never blocks the job; Trivy is the real gate. The NVD CVE database (`~/.m2/repository/org/owasp/dependency-check-data`) is cached via `actions/cache` across runs - without it every run re-downloads the full ~390k-record dataset (20-40+ min).
- **`.trivyignore`** (repo root): suppresses CVEs Trivy correctly finds but that aren't ours to fix - currently the `usr/bin/pebble` findings baked into the `eclipse-temurin` base image (Canonical's service supervisor, statically-compiled Go binary, not app code). Each entry has an `exp:` date to force a re-check once the base image might have caught up - don't just bump the date without re-verifying.
- **Deploy job**: active since 2026-09-12 - every green push to `main` SSHes into the shared VPS (`VPS_HOST`/`VPS_USER`/`VPS_SSH_KEY` repo secrets) and runs `git pull && docker compose build backend frontend && docker compose up -d` in `/home/kevinspace/almo`. If `appleboy/ssh-action` fails with `ssh: no key found`, the `VPS_SSH_KEY` secret's line breaks got mangled on paste (common copy-paste issue with multi-line PEM/OpenSSH keys) - re-paste the exact key content, don't regenerate a new keypair unless the old one is actually gone.

## Deployment (VPS)

Routine deploys (pull + rebuild + restart) now happen automatically via the CI deploy job above on every green push to `main` - no manual SSH needed for that anymore. Anything beyond that (nginx config, DNS, TLS, first-time server setup) is still out of scope for a normal dev workflow - covered in the spec's "Deployment / Infra" section and `infra/nginx/almo-group.vn-nspace.de.conf`. Requires root/SSH access to the shared VPS; not something to run from a local machine. Ask before touching anything server-side, per CLAUDE.md's team-coordination rules.
